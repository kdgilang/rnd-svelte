<script>
  ...
  import {PUBLIC_FLIP_API_HOST, PUBLIC_FLIP_API_KEY, PUBLIC_LOCAL_API_HOST} from '$env/static/public';
  ...
  $: {
    $cartStore?.items?.map(item => {
      subtotalAmount += ((item?.product?.price?.amount + item?.topping?.price?.amount + item?.cream?.price?.amount) * item.qty)
    })
          ...
          <div class="justify-between mb-6 rounded text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-700 p-6 shadow-md sm:flex sm:justify-start">
            ...
            <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
              <div class="mt-5 sm:mt-0">
                ...
                <p class="mt-1 text-xs">cream: {item?.cream?.title} ({item?.cream.price.idrFormatted})</p>
                <p class="mt-1 text-xs">topping: {item?.topping?.title || '-'} ({item?.topping.price.idrFormatted})</p>
              </div>
              <div class="mt-4 pl-2 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
      ...
      <div class="mt-6 h-full rounded text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-700 p-6 shadow-md md:mt-0 md:w-1/3">
        {#each $cartStore.items as item}
          <div class="mb-2 flex justify-between">
            ...
            <p>
              {priceFormatted(
              (item?.product?.price?.amount + item?.topping?.price?.amount + item?.cream?.price?.amount)*item.qty
              )}
            </p>
          </div>
          <hr class="my-4" />
